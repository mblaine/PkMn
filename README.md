...
* **PkMn.Model** - Reads in static information on Pokémon species and moves and makes it available to be used by the rest of the program. See also: the XML files under [PkMn.Model\Generation-I](tree/master/PkMn.Model/Generation-I).
* **PkMn.Instance** - Uses the data provided by PkMn.Model to caluculate the stats for indiviual Pokémon and orchestrate battles. See also: the classes [``Battle``](tree/master/PkMn.Instance/Battle.cs), [``ActiveMonster``](tree/master/PkMn.Instance/ActiveMonster.cs), and [``Monster``](tree/master/PkMn.Instance/Monster.cs).
